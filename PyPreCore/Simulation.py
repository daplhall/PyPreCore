...
class Particles(object):
    ...
    def __init__(self, n, env, R, Stmin, Stmax, rho_d=1.6, C=0.004):
        ...
        tff = np.sqrt(
            3 * np.pi / (32 * self.units.G * self.env.rho(rmag))
        )  # TODO move to enviorment
        vth = np.sqrt(
            8
            / np.pi
            * self.units.k_B
            * self.env.T(rmag)
            / (self.env.mu * self.units.m_p)
        )  # TODO move to enviorment instead, and then just call it.
    ...
    def courant(
        self, Cdr=0.2, Cff=0.1, Csnap=0.1, Ckep=0.1, same_dt=False, verbose=False
    ):
        ...
        dt_dr = Cdr * np.sqrt(
            2 * dr / magnitude(a, True)
        )  # NOTE a is the total acceleration ie dv/dt that is solve for velocity
        if np.any(self.env.rho(rm) == 0):
            debug = True
        dt_ff = Cff * np.sqrt(
            3 * np.pi / (32 * self.units.G * self.env.rho(rm))
        )  # free fall time.
        dt_snap = Csnap * self.env.dtsnap  # Snapshot based
        dt_kep = Ckep * np.sqrt(
            rm * rm * rm / (self.units.G * self.env.M(rm))
        )  # kepler based
        ...
        if verbose:
            ...
            print(
                "idx: %i,  radius: %.3e, cell: %i, dt_dr: %.3e, dt_ff: %.3e, dt_snap: %.3e, dt_kep: %.3e, size: %.3e, a_grav: %.3e"
                % (
                    imin,
                    magnitude(self.r)[imin] / self.units.AU,
                    self.search[imin],
                    dt_dr[imin] / self.units.yr,
                    dt_ff[imin] / self.units.yr,
                    dt_snap / self.units.yr,
                    dt_kep[imin] / self.units.yr,
                    self.s[imin],
                    a[imin][0],
                )
            )
    ...
    def KDK_drag(self, same_dt=True):
        ...
        Cdt = (
            0.5 * self.dt * self.env.drag(r, self.s, self.rho_d)
        )  # NOTE changed self.r to r as it should be at time step 0.5
...
class DataSphere(object):
    ...
    def update(self):
        ...
        interpolateM = [
            scilate.interp1d(
                self._re[:, idx],
                np.cbrt(self._M[:, idx]),
                fill_value=(np.cbrt(self._M[0, idx]), np.cbrt(self._M[-1, idx])),
                bounds_error=False,
                kind="linear",
            )
            for idx in range(self.nsteps)
        ]  # re instead of rc i misunderstoond
        self.interpolateM = [cube_function(func) for func in interpolateM]
        self.interpolateT = [
            scilate.interp1d(
                self._rc[:, idx],
                self._T[:, idx],
                fill_value=(self._T[0, idx], 0),
                bounds_error=False,
            )
            for idx in range(self.nsteps)
        ]
        self.interpolatesearch = [
            scilate.interp1d(
                self._re[:, idx], n, fill_value=(0, self.ncells - 1), bounds_error=False
            )
            for idx in range(self.nsteps)
        ]
        self.interpolateCS = [
    ...
    def next(self):
        self.idstep = (
            self.idstep + 1 if self.nsteps - 1 > self.idstep else self.nsteps - 1
        )
...
def Particles_grain_radial(
    sphere: DataSphere, func_dist, grain_interface, D=0.01, debug=False, **kwards
):
    """
...
class DataParticles(object):
    ...
    def __init__(self, loadpath, env, skiprows=0, max_rows=None):
        self.dt = np.loadtxt(
            loadpath + "\\dt.txt", skiprows=skiprows, max_rows=max_rows
        )
        self.M = np.loadtxt(
            loadpath + "\\mass.txt", skiprows=skiprows, max_rows=max_rows
        )
        self.r = np.loadtxt(
            loadpath + "\\radius.txt", skiprows=skiprows, max_rows=max_rows
        )
        self.s = np.loadtxt(loadpath + "\\s.txt", skiprows=skiprows, max_rows=max_rows)
        self.t = np.loadtxt(
            loadpath + "\\time.txt", skiprows=skiprows, max_rows=max_rows
        )
        self.vg = np.loadtxt(
            loadpath + "\\vel.txt", skiprows=skiprows, max_rows=max_rows
        )
        self.env = env
        self.units = env.units
        # interpolation arrays
...
class Particles_Corrected(Particles):
    ...
    def r_trans(self):
        ...
        rmask = (
            rp <= self.env.rc[idx]
        )  # TODO NOTE this changed this here to force beta to be 1 for rp < ra for idx == 0
        ...
        Dt = (
            self.t[:, 0] - self.env.t[self.env.idstep]
        )  # TODO assign this as a self. attribute
        Dr = self.env.rc[1:] - self.env.rc[:-1]  # TODO as above
        Dvg = (
            self.env._vg[1:, self.env.idstep] - self.env._vg[:-1, self.env.idstep]
        )  # NOTE here is a problem as the last cell ie dvg may be wrong

        beta = 1 / (1 + Dvg[idx] * Dt / Dr[idx])
        beta[(rmask) & (idx == 0)] = (
            1  # NOTE changed to one swe we just get a normal translation.
        )
        ### small hack for idx = ncells -1
        beta[rup] = 1
        ####
        ...
        res = (
            (beta * rp + (1 - beta) * ra - Dt * beta * vga)[:, None]
            * self.r
            / magnitude(self.r, True)
        )
        return res
    ...
    def kick(self, step=0.5):
        ...
        force = self.gravity()  # not really a force, more so an acceleration
        self.v[self.runmask] = (
            self.v[self.runmask] + step * self.dt[self.runmask] * force
        )
        # return             step*self.dt*force
    ...
    def drift(self, step=1.0):
        ...
        """
        self.r[self.runmask] = (
            self.r[self.runmask] + step * self.dt[self.runmask] * self.v[self.runmask]
        )
        # return             step*self.dt*self.v
    ...
    def KDK_drag_Corrected(self, same_dt=True):
        ...
        Cdt = (
            0.5
            * self.dt[self.runmask]
            * self.env.drag(r, self.s[self.runmask], self.rho_d)
        )
        self.v[self.runmask] = self.v[self.runmask] - Cdt * (
            self.v[self.runmask] - self.env.vg(r)
        ) / (1 + Cdt)
        self.drift(1.0)
        self.kick(0.5)
        ...
        self.v[self.runmask] = self.v[self.runmask] - Cdt * (
            self.v[self.runmask] - self.env.vg(rtrans)
        ) / (1 + Cdt)
    ...
    def KDK_drag_Corrected_collected(self, same_dt=True):
        ...
        rm = magnitude(self.r, True)
        dtgrav = (
            0.5 * self.dt * (-self.r * self.units.G * self.env.M(rm) / (rm * rm * rm))
        )
...
def grain_dist_radius_Correction(
    sphere: DataSphere,
    func_dist,
    grain_interface,
    D=0.01,
...
def dust_to_gas_DataParticles(p, size: float, snapid: int):
    ...
    dm_g = Mg[1:] - Mg[:-1]
    rc = (
        p.env.re[:-1] + 0.5 * p.env.dr
    )  # p.env._rc # rc     = p.env._re[:-1,:] + 0.5*p.env.dr[:,None]
    # m_dust_bin = [np.sum(p.M[s_low][bins == i]) for i in bins] #
    m_dust_bin = np.zeros_like(dm_g)
    farray = np.zeros_like(dm_g)
    Mlost = 0
    for pm, r, sbin in zip(p.M[snapid, s_low], p.r[snapid, s_low], bins):
        ...
        if r <= rc[sbin]:
            ...
            if (
                sbin == p.env.ncells - 2
            ):  # its -2 because p.env.ncells i counted, so it effectivly start from 1, so we have ncells-1
...
def CalcDustmass_snapid(p: DataParticles, s, snapid):
    ...
    rc = rc[:, snapid]
    bins = p.search(p.r)
    for idx, pm, r, sbin in enumerate(
        zip(p.M[snapid, smask], p.r[snapid, smask], bins)
    ):
        if r <= rc[sbin]:
            if sbin == 0:
                f[idx] = 0
...
def CalcDustmass_size(p: DataParticles, s):
    ...
    f = np.zeros(p.n, dtype=np.float64)
    neighbor = np.zeros(p.n, dtype=np.int64)
    dustmass_bin = np.zeros(
        p.env.ncells - 1, dtype=np.float64
    )  # should be cell- 2 long
...
def InitialMassCorrection(p, fj, D, max_iter=100, diff_eps=1e-15, verbose=False):
    ...
    for ids, ss in enumerate(s_unique):
        ...
        while k < max_iter:  # ((d2g.max() - d2g.min()) > diff_eps)
            ...
            p.M[smask] = (
                (1 - f[smask]) * FF_bin[smask] + f[smask] * FF_neigh[smask]
            ) * p.M[smask]
            cell_mass, f, bins, neighbor = CalcDustmass_size(p, ss)
            d2g = cell_mass / gas_mass
    ...
    if verbose:
        print(
            "Old total mass: %.4e" % (Mold / p.units.m_Sun),
            "m_sun ,New total mass: %.4e" % (p.M.sum() / p.units.m_Sun),
            " m_sun",
        )
...
class save_particles(object):
    def __init__(self, p: Particles, savepath: str):
        ...
        try:
            ...
            inputt = input(
                "File Folder exists, files might be interfered with, want to continure (y/n)"
            ).lower()
            if inputt != "y":
...
def main():
    ...
    p, fj, D = grain_dist_radius_Correction(
        sphere, GrainDistIntegrated, ginterface, debug=True, repeat=4
    )
    p.v[:, 1] = kep_percent * np.sqrt(
        p.units.G * p.env.M(magnitude(p.r)) / magnitude(p.r)
    )  # NOTE assumes that p.v is ineritrely 0
...
def sim(folder, kep, spherefolder):
    ...
    s0, s1 = (
        microns_to_cm(21),
        microns_to_cm(117),
    )  # 1.42921456e-03, 1.42921456e-01 # <- already in cm
    ...
    ##-------##
    vkep_dist = scilate.interp1d(
        [si, sj],
        [vkep1, vkep2],
        kind="linear",
        bounds_error=False,
        fill_value=(vkep1, vkep2),
    )
    ##-------##
