...
class Particles(object):
    ...
    def courant(self, Cdr = 0.2, Cff = 0.1, Csnap = 0.1, Ckep = 0.1, same_dt = False):
        ...
        dt_snap = Csnap * p.env.dtsnap                                          # Snapshot based 
        dt_kep  = Ckep  * np.sqrt(rm*rm*rm/(p.units.G*p.env.M(rm)))             # kepler based
...
# %%
np.all((sphere.re[1:]-sphere.re[:-1]) == sphere.dr)


# %%
sphere._re[1:,:] - sphere._re[:-1,:] 


# %%
(sphere._re[1:,:] - sphere._re[:-1,:] ).shape
